...
class TestDeviceDefinition:
    def test_valid_device_definition(self) -> None:
        ...
        device = DeviceDefinition(
            id="LiquidHandler-01",
            driver_type="SiLA2",
            endpoint=cast(HttpUrl, "https://192.168.1.50:50052"),
    ...
    def test_invalid_url(self) -> None:
        ...
        with pytest.raises(ValidationError) as excinfo:
            DeviceDefinition(
                id="Device-01",
                driver_type="SiLA2",
                endpoint=cast(HttpUrl, "not-a-url"),
                capabilities=[],
    ...
    def test_url_edge_cases(self) -> None:
        ...
        device_ipv6 = DeviceDefinition(
            id="IPv6-Dev",
            driver_type="SiLA2",
            endpoint=cast(HttpUrl, "http://[::1]:8080"),
            capabilities=[],
            edge_agent_model="model.onnx",
            allowed_reflexes=[],
        )
        ...
        device_query = DeviceDefinition(
            id="Query-Dev",
            driver_type="SiLA2",
            endpoint=cast(HttpUrl, "https://api.lab.com/v1/device?token=abc"),
            capabilities=[],
            edge_agent_model="model.onnx",
            allowed_reflexes=[],
        )
        ...
        device_auth = DeviceDefinition(
            id="Auth-Dev",
            driver_type="SiLA2",
            endpoint=cast(HttpUrl, "https://user:pass@192.168.1.50"),
            capabilities=[],
            edge_agent_model="model.onnx",
            allowed_reflexes=[],
        )
    ...
    def test_invalid_onnx_extension(self) -> None:
        ...
        with pytest.raises(ValidationError) as excinfo:
            DeviceDefinition(
                id="Device-01",
                driver_type="SiLA2",
                endpoint=cast(HttpUrl, "https://localhost:8080"),
                capabilities=[],
    ...
    def test_serialization_roundtrip(self) -> None:
        ...
        device = DeviceDefinition(
            id="Complex-Dev",
            driver_type="SiLA2",
            endpoint=cast(HttpUrl, "https://192.168.1.1:5000/path"),
            capabilities=["A", "B"],
...
class TestSoftSensorModel:
    ...
    def test_missing_fields(self) -> None:
        ...
        with pytest.raises(ValidationError):
            # We use cast(Any, ...) to bypass static checks for missing arguments
            # forcing a runtime check by Pydantic
            SoftSensorModel(
                **cast(
                    Any,
                    {
    ...
    def test_invalid_type_artifact(self) -> None:
        ...
        with pytest.raises(ValidationError) as excinfo:
            SoftSensorModel(
                ...
                target_variable="y",
                physics_constraints={},
                model_artifact=cast(bytes, 12345),
            )
